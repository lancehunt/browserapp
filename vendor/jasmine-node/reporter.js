  ...
  reportSuiteResults: function(suite) {
    ...
    specResults.items_.forEach(function(spec){
      if (spec.failedCount > 0 && spec.description) {
        ...
        spec.items_.forEach(function(result){
          if (!result.passed_) {
            if(outerThis.teamcity_) {
              ...
              outerThis.log_.push(outerThis.stringWithColor_('  F  ' +  outerThis.stackFilter(result.trace.stack) + '\n', outerThis.color_.fail()));
            }
          }
        });
        ...
        if (outerThis.isVerbose_ && !outerThis.teamcity_)
          outerThis.log_.push(outerThis.stringWithColor_('  √', outerThis.color_.pass()) + '  it ' + spec.description);
      }
    });
